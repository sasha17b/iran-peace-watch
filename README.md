# Iran Private Equity Fund: Readiness for First LP Talks
### A daily situation report tracking implementation of the **Islamabad Memorandum** and the trajectory toward a final peace settlement to be signed in Geneva.

`As of 21 Jun 2026`  ·  `Settlement clock: Day 4 of 60`  ·  `Readiness: 🔴 ~20 / 100`  ·  `Basis: open-source reporting`

---

## Bottom line

> **Window opening — not yet open.** The Iran settlement is real but fragile, and the variable our target LPs (Partners Group, LGT Capital Partners) will actually underwrite — *legally durable, non-reversible sanctions relief* — has **not** triggered. The ceasefire holds under strain; no sanctions-relief schedule yet exists.
>
> **So what for OUDRA:** advance the fund build on the critical path now, but **hold formal LP outreach** until the settlement proves durable.
>
> **Readiness for first LP talks** &nbsp; 🔴 &nbsp; **~20 / 100 — not ready.** &nbsp; Threshold to open informal talks ~75; to open a formal raise ~90.

---

## Readiness for first LP talks — conditions & comfort score

> **Comfort score: 🔴 ~20 / 100 — not ready.** The market preconditions that make a Partners Group / LGT CP conversation credible are largely unmet. Threshold to open first *informal* talks: **~75 / 100**; to open a formal raise: ~90.

What must hold true before a credible first conversation. Progress scale: ○ 0 · ◔ 25 · ◑ 50 · ◕ 75 · ● 100

| Condition | Why PG / LGT CP need it | Progress |
|---|---|:--:|
| Ceasefire durable, no resumption of war | No allocator underwrites an active conflict | ◔ 25 |
| Final framework deal signed and holding | Confirms a real political path exists | ◑ 50 |
| Credible, scheduled sanctions-relief pathway | The legal gate to deploying capital | ○ 0 |
| Signalled legal irreversibility (UNSC track) | Protection against snap-back risk | ○ 0 |
| Emerging capital convertibility / repatriation | LPs must be able to get money out | ◔ 25 |

**Score bands:** 0–25 not ready · 26–50 early preparation · 51–75 approaching · 76–90 ready for first talks · 91–100 ready to raise.

**Binding constraint:** the two deal-breakers — a credible sanctions-relief pathway and signalled legal irreversibility — sit at zero, and both move only with the politics. The comfort score cannot approach the talks threshold until these turn.

---

## Settlement de-risking checklist — the fourteen points

RAG: 🟢 implemented · 🟡 in progress / contested · 🔴 breached · ⚪ not started

#### I. Cessation of hostilities
| # | Provision | | Note |
|:--:|---|:--:|---|
| 1 | End all military operations on every front, incl. Lebanon | 🔴 | Israeli strikes in south Lebanon (20 Jun) killed ≥16. Tehran calls it a breach; Iran's lead demand at Bürgenstock. |
| 2 | Mutual sovereignty and non-interference | ⚪ | Declaratory; untested. |

#### II. Negotiating architecture
| # | Provision | | Note |
|:--:|---|:--:|---|
| 3 | Final deal within 60 days (extendable) | 🟡 | Clock running; Day 4 of 60. |
| 9 | Hold the status quo during talks | 🟡 | Undercut by Lebanon strikes and the Hormuz dispute. |
| 12 | Stand up a joint monitoring / compliance body | ⚪ | Not yet constituted. |
| 13 | Phased negotiations once initial provisions function | 🟡 | 21 Jun: first round convened at Bürgenstock (US–Iran, Pakistani & Qatari mediators). No confirmed readout. |
| 14 | Final deal endorsed by binding UNSC resolution | ⚪ | **Key LP-diligence item.** Contingent on a final deal. |

#### III. Maritime normalisation
| # | Provision | | Note |
|:--:|---|:--:|---|
| 4 | US lifts naval blockade (complete within 30 days) | 🟡 | Mediator says it ceases immediately; some reporting says it persists. |
| 5 | Iran reopens Hormuz, toll-free, 60 days | 🔴 | Iran declared the strait closed (20 Jun); US CENTCOM reports it open — 55 vessels, 17m+ bbl transited Saturday. |

#### IV. Economic provisions
| # | Provision | | Note |
|:--:|---|:--:|---|
| 6 | ≥ $300bn reconstruction fund (mechanism within 60 days) | ⚪ | **Defines the opportunity set.** Tied to the final deal. |
| 7 | Terminate all US sanctions on an agreed schedule | ⚪ | **The investable trigger.** Tied to the final deal. |
| 10 | Immediate Treasury waivers for oil exports & related finance | 🟡 | "Immediate" per text; issuance not independently confirmed. |
| 11 | Release Iranian frozen assets on implementation | ⚪ | Procedures to be agreed. |

#### V. Nuclear file
| # | Provision | | Note |
|:--:|---|:--:|---|
| 8 | No weapons; enriched stock down-blended on-site under IAEA | ⚪ | No cap, no figures, no enforcement; deferred to the 60-day window. |

---

## Daily assessment log

| Date | Call | Net read of the day |
|---|:---:|---|
| **21 Jun** | 🟡 | Bürgenstock talks convened — first since signing — after a morning of cancel-then-reinstate. Nuclear-first vs. Lebanon-first split; Hormuz still contested. Step forward, no breakthrough. |
| **20 Jun** | 🔴 | Iran declares Hormuz closed over Lebanon strikes; Israeli strikes kill ≥16; talks in doubt. |
| **19 Jun** | 🔴 | First technical round called off; US delegation shelves travel amid the Lebanon escalation. |
| **18 Jun** | 🟢 | Pakistan signals the US blockade will lift and Hormuz reopen under the framework. |
| **17 Jun** | 🟢 | Memorandum signed (Versailles / Tehran). War formally ends; 60-day clock begins. |

---

<sub>**Method.** Settlement status compiled from open-source reporting (Reuters, AP, NPR, Al Jazeera, CNN, Times of Israel); contested claims flagged. The comfort score is a 0–100 readiness index built from the market preconditions for a credible first LP conversation, where the investable trigger is the onset of durable, non-reversible sanctions relief (provisions 7 + 14). Updated on demand; each entry is dated.</sub>
